...
export default function SecProjectsAdminPage() {
  ...
  const fetchProjects = async () => {
    setIsLoading(true);
    try {
      const q = query(collection(db, "secProjects"), orderBy("title"));
      ...
      console.error("Error fetching SEC projects: ", error);
      toast({ title: "Xəta", description: "ŞEC layihələrini yükləyərkən problem yarandı.", variant: "destructive" });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleFormSubmit = async (values: Omit<ProjectArticle, "id">, imageFile: File | null) => {
    try {
      ...
      if (editingProject && editingProject !== 'new') {
        const projectRef = doc(db, "secProjects", editingProject.id);
        await updateDoc(projectRef, dataToSave);
        toast({ title: "Uğurlu", description: "ŞEC Layihəsi yeniləndi." });
      } else {
        await addDoc(collection(db, "secProjects"), dataToSave);
        toast({ title: "Uğurlu", description: "Yeni ŞEC layihəsi əlavə edildi." });
      }
  ...
  const handleDelete = async (projectId: string) => {
    try {
      await deleteDoc(doc(db, "secProjects", projectId));
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold">ŞEC Layihələrini İdarə Et</h1>
          <p className="text-muted-foreground">Şagird Elmi Cəmiyyəti üçün layihələr əlavə edin, redaktə edin və ya silin.</p>
        </div>
       ...
       {editingProject && (
        <Card className="mb-8">
          <CardHeader>
            <CardTitle>{editingProject === 'new' ? "Yeni ŞEC Layihəsi Yarat" : "Layihəni Redaktə Et"}</CardTitle>
            <CardDescription>
      ...
      <Card>
        <CardHeader>
          <CardTitle>ŞEC Layihələr Siyahısı</CardTitle>
