...
const formSchema = z.object({
  ...
  faculty: z.string().min(5, { message: "Fakültə adı ən azı 5 simvoldan ibarət olmalıdır." }),
...
interface TeamFormProps {
  onSubmit: (values: Omit<TeamMember, 'id'>, imageFile: File | null) => Promise<void>;
  initialData?: TeamMember | null;
  onClose: () => void;
}

export function TeamForm({ onSubmit, initialData, onClose }: TeamFormProps) {
  ...
  const handleSubmit = async (values: FormValues) => {
    ...
    try {
      await onSubmit(values as Omit<TeamMember, 'id'>, imageFile);
      form.reset();
      onClose();
    } catch (error: any) {
      toast({
        ...
        <ScrollArea className="flex-1 pr-4">
          <div className="space-y-6 pr-2">
            <div className="grid grid-cols-3 gap-4">
             ...
             <FormField
                ...
                render={({ field }) => (
                    ...
                    <FormControl><Input placeholder="Məs., Tədbirlər Koordinatoru" {...field} /></FormControl>
                    <FormMessage />
                    </FormItem>
                )}
                />
            ...
            render={({ field }) => (
                ...
                <FormControl><Input placeholder="Təhsil aldığı və ya rəhbərlik etdiyi fakültə" {...field} /></FormControl>
                <FormMessage />
                </FormItem>
            )}
            />
            ...
            render={({ field }) => (
                ...
                <FormControl><Textarea placeholder="Komanda üzvü haqqında ətraflı məlumat" {...field} rows={5} /></FormControl>
                <FormMessage />
                </FormItem>
            )}
            />
            ...
            <h3 className="text-lg font-medium pt-4 border-t">Sosial Media Linkləri</h3>
