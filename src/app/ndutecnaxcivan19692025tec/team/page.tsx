...
import { TeamForm, TeamMember } from "./team-form";
...
export default function TeamAdminPage() {
  ...
  const fetchMembers = async () => {
    ...
    try {
      ...
      toast({ title: "Xəta", description: "Komanda üzvlərini yükləyərkən problem yarandı.", variant: "destructive" });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleFormSubmit = async (values: Omit<TeamMember, "id">, imageFile: File | null) => {
    try {
      ...
      if (editingMember && editingMember !== 'new') {
        ...
        await addDoc(collection(db, "teamMembers"), dataToSave);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold">Komandanı İdarə Et</h1>
          <p className="text-muted-foreground">Yeni üzvlər əlavə edin, redaktə edin və ya silin.</p>
        </div>
       ...
       {editingMember && (
        <Card className="mb-8">
          <CardHeader>
            <CardTitle>{editingMember === 'new' ? "Yeni Üzv Yarat" : "Üzvü Redaktə Et"}</CardTitle>
            <CardDescription>
                Komanda üzvünün məlumatlarını daxil edin və ya yeniləyin.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <TeamForm
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>"Haqqımızda" səhifəsində görünən üzvlər.</CardDescription>
        </CardHeader>
        <CardContent>
           {isLoading ? (
