...
export default function SecBoardAdminPage() {
  ...
  const fetchMembers = async () => {
    ...
    try {
      ...
      toast({ title: "Xəta", description: "ŞEC idarə heyətini yükləyərkən problem yarandı.", variant: "destructive" });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold">ŞEC İdarə Heyətini İdarə Et</h1>
          <p className="text-muted-foreground">Şagird Elmi Cəmiyyətinin idarə heyətinə yeni üzvlər əlavə edin, redaktə edin və ya silin.</p>
        </div>
       ...
       {editingMember && (
        <Card className="mb-8">
          <CardHeader>
            <CardTitle>{editingMember === 'new' ? "Yeni İdarə Heyəti Üzvü Yarat" : "Üzvü Redaktə Et"}</CardTitle>
            <CardDescription>
                İdarə heyəti üzvünün məlumatlarını daxil edin və ya yeniləyin.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <SecTeamForm
      ...
      <Card>
        <CardHeader>
          <CardTitle>ŞEC İdarə Heyəti Üzvləri</CardTitle>
        ...
        <CardContent>
           {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-48 text-center text-muted-foreground border-2 border-dashed rounded-lg">
                ...
                <h3 className="text-xl font-semibold">Heç bir idarə heyəti üzvü tapılmadı.</h3>
            </div>
           )}
        </CardContent>
      </Card>
