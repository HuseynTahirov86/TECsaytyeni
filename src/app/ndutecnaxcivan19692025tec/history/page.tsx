...
export default function HistoryAdminPage() {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          setContent(docSnap.data().historyContent || '');
        }
      } catch (error) {
  ...
  const handleFormSubmit = async (values: { content: string }) => {
    try {
      await updateDoc(docRef, { historyContent: values.content });
      toast({ title: "Uğurlu", description: "Tariximiz səhifəsi yeniləndi." });
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-3xl font-bold">"Tariximiz" Səhifəsi</h1>
