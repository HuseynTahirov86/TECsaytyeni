...
export interface AcademicWritingRule {
  id: string;
  imageUrl: string;
  order: number;
}
...
interface AcademicWritingRuleFormProps {
  onSubmit: (values: Omit<AcademicWritingRule, 'id'>, imageFile: File | null) => Promise<void>;
  initialData?: AcademicWritingRule | null;
  onClose: () => void;
}

export function AcademicWritingRuleForm({ onSubmit, initialData, onClose }: AcademicWritingRuleFormProps) {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
        ...
        <FormField
          ...
          render={({ field }) => (
              <FormItem>
              <FormLabel>Qayda Şəkli</FormLabel>
               <FormControl>
                <ImageUploader 
                  id="rule-image"
              ...
              <FormDescription>Qayda üçün şəkli yükləyin.</FormDescription>
              <FormMessage />
              </FormItem>
          )}
        />
