...
export default function ScienceJournalAdminPage() {
  const [submissions, setSubmissions] = useState<ScienceSubmission[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchSubmissions = async () => {
      setIsLoading(true);
      try {
        const q = query(collection(db, "scienceSubmissions"), orderBy("submittedAt", "desc"));
        const querySnapshot = await getDocs(q);
        const submissionList = querySnapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data(),
        }) as ScienceSubmission);
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteDoc(doc(db, "scienceSubmissions", id));
  ...
  const handleApprove = async (id: string) => {
    try {
      const submissionRef = doc(db, "scienceSubmissions", id);
      await updateDoc(submissionRef, { status: 'təsdiqləndi' });
      toast({ title: "Uğurlu", description: "Məqalə təsdiqləndi." });
      const updatedSubmissions = submissions.map(s => s.id === id ? { ...s, status: 'təsdiqləndi' } as ScienceSubmission : s);
  ...
  const handleExport = () => {
    ...
    exportToExcel(dataToExport, "Elmi_Jurnal_Məqalələri");
  };
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-3xl font-bold">Tələbə Elmi Jurnalı</h1>
                           ...
                           <div className="space-y-4">
                             <div className="flex justify-between items-start">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2 text-sm">
                                    ...
                                    <p className="flex items-center gap-2 font-semibold"><FileText className="h-4 w-4 text-primary"/>Elm sahəsi:</p>
                                     <p>{submission.fieldOfScience}</p>
