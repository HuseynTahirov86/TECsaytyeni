...
export default function SecMessagesAdminPage() {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      setIsLoading(true);
      try {
        const q = query(collection(db, "secContacts"), orderBy("sentAt", "desc"));
        ...
        console.error("Error fetching SEC messages: ", error);
  ...
  const handleDelete = async (messageId: string) => {
    try {
      await deleteDoc(doc(db, "secContacts", messageId));
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-3xl font-bold">ŞEC Gələnlər Qutusu</h1>
        <p className="text-muted-foreground">ŞEC əlaqə forması vasitəsilə göndərilən mesajlar.</p>
      </div>
      <Card>
        <CardHeader>
          <CardTitle>Mesajlar Siyahısı</CardTitle>
          <CardDescription>Burada ŞEC bölməsindən göndərilən bütün müraciətləri görə bilərsiniz.</CardDescription>
        </CardHeader>
        <CardContent>
            {isLoading ? (
