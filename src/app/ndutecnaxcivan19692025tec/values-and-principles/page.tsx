...
export default function ValuesAndPrinciplesAdminPage() {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          setContent(docSnap.data().valuesContent || '');
        }
      } catch (error) {
  ...
  const handleFormSubmit = async (values: { content: string }) => {
    try {
      await updateDoc(docRef, { valuesContent: values.content });
      toast({ title: "Uğurlu", description: "Dəyər və Prinsiplər səhifəsi yeniləndi." });
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-3xl font-bold">"Dəyər və Prinsiplər" Səhifəsi</h1>
