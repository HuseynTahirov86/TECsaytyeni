...
export default function StrategicGoalsAdminPage() {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          setContent(docSnap.data().goalsContent || '');
        }
      } catch (error) {
  ...
  const handleFormSubmit = async (values: { content: string }) => {
    try {
      await updateDoc(docRef, { goalsContent: values.content });
      toast({ title: "Uğurlu", description: "Strateji məqsədlər səhifəsi yeniləndi." });
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-3xl font-bold">"Strateji Məqsədlər" Səhifəsi</h1>
