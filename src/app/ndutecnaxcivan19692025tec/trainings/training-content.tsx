...
import { ArrowRight, CheckCircle, Trophy, XCircle, Loader2 } from 'lucide-react';
import { completeTrainingAndGenerateCertificate, saveQuizResult } from '@/lib/actions';
...
export function TrainingContent({ training, registration, user, onComplete }: TrainingContentProps) {
  ...
  useEffect(() => {
    // If quiz is already completed and passed, move to the last step to show result
    if (registration?.quizResult && (registration.quizResult.score / registration.quizResult.total) >= 0.8) {
  ...
  const handleQuizCompletion = async (answers: Record<number, number>, score: number, total: number) => {
    ...
    try {
        ...
        // Check if score is >= 80% to complete the training
        if ((score / total) < 0.8) {
          toast({
              title: "Məyusedici Nəticə",
              description: `Sertifikat əldə etmək üçün ən az 80% nəticə göstərməlisiniz. Sizin nəticəniz: ${Math.round((score/total) * 100)}%`,
              variant: "destructive",
              duration: 7000,
          });
          onComplete(); // Still refresh parent to show saved (failed) result
          return;
        }

        const completionResult = await completeTrainingAndGenerateCertificate(registration.id);
        ...
        <div className="mt-8">
            ...
            {isQuizCompleted && (
              <div className="space-y-6 pt-8 mt-8 border-t-2 border-dashed">
                { registration?.status === 'tamamlandı' && training.hasCertificate && registration?.certificateId ? (
                   <>
                ...
                ) : quizResult.score / quizResult.total >= 0.8 && registration?.status === 'tamamlandı' && !training.hasCertificate ? (
                      ...
                      <Alert variant="destructive">
                          ...
                          <AlertTitle>Daha Çox Çalışmalısınız</AlertTitle>
                          <AlertDescription>
                              Nəticəniz {Math.round((quizResult.score/quizResult.total) * 100)}% təşkil etdi. Sertifikat əldə etmək üçün təlimi yenidən nəzərdən keçirib, testi təkrar işləyə bilərsiniz.
                          </AlertDescription>
                      </Alert>
                   )
                }
                 <Quiz 
                    ...
                    isCompleting={isCompleting}
                    registrationId={registration.id!}
                    initialAnswers={registration?.quizResult?.answers}
                  />
              </div>
            )}
        </div>
