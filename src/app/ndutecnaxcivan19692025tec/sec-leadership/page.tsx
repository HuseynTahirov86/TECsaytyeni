...
export default function SecLeadershipAdminPage() {
  ...
  const fetchMembers = async () => {
    ...
    try {
      ...
      setMembers(memberList.filter(m => m.role === 'Sədr'));
    } catch (error) {
      console.error("Error fetching SEC leadership: ", error);
      toast({ title: "Xəta", description: "ŞEC rəhbərliyini yükləyərkən problem yarandı.", variant: "destructive" });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleFormSubmit = async (values: Omit<SecTeamMember, "id">, imageFile: File | null) => {
    try {
      ...
      if (editingMember && editingMember !== 'new') {
        ...
        toast({ title: "Uğurlu", description: "Rəhbər məlumatı yeniləndi." });
      } else {
        await addDoc(collection(db, "secTeamMembers"), dataToSave);
        toast({ title: "Uğurlu", description: "Yeni rəhbər əlavə edildi." });
      }
  ...
  const handleDelete = async (memberId: string) => {
    try {
      ...
      toast({ title: "Uğurlu", description: "Rəhbər silindi." });
      await fetchMembers();
    } catch (error) {
      console.error("Error deleting member: ", error);
      toast({ title: "Xəta", description: "Rəhbəri silərkən problem yarandı.", variant: "destructive" });
    }
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold">ŞEC Rəhbərliyini İdarə Et</h1>
          <p className="text-muted-foreground">Şagird Elmi Cəmiyyətinin sədrini idarə edin.</p>
        </div>
        <Button onClick={() => setEditingMember(editingMember === 'new' ? null : 'new')}>
          <PlusCircle className="mr-2 h-4 w-4" />
          {editingMember === 'new' ? "Formu Bağla" : "Yeni Rəhbər Əlavə Et"}
        </Button>
      </div>
       ...
       {editingMember && (
        <Card className="mb-8">
          <CardHeader>
            <CardTitle>{editingMember === 'new' ? "Yeni Rəhbər Yarat" : "Rəhbəri Redaktə Et"}</CardTitle>
            <CardDescription>
                ŞEC sədrinin məlumatlarını daxil edin və ya yeniləyin.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <SecTeamForm
      ...
      <Card>
        <CardHeader>
          <CardTitle>ŞEC Rəhbərliyi</CardTitle>
          <CardDescription>"ŞEC Haqqında" səhifəsində rəhbər kimi görünən şəxs.</CardDescription>
        </CardHeader>
        <CardContent>
           {isLoading ? (
                           ...
                           <div className="p-4 bg-muted/50 rounded-b-md flex items-center gap-2">
                                ...
                                <AlertDialog>
                                    ...
                                    <AlertDialogContent>
                                        ...
                                        <AlertDialogDesc>
                                            Bu əməliyyat geri qaytarıla bilməz. Bu, rəhbəri sistemdən həmişəlik siləcək.
                                        </AlertDialogDesc>
                                        </AlertDialogHeader>
                                        <AlertDialogFooter>
            ...
            <div className="flex flex-col items-center justify-center h-48 text-center text-muted-foreground border-2 border-dashed rounded-lg">
                ...
                <h3 className="text-xl font-semibold">Heç bir rəhbər tapılmadı.</h3>
            </div>
           )}
        </CardContent>
      </Card>
