...
export default function MainActivitiesAdminPage() {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          setContent(docSnap.data().missionContent || '');
        }
      } catch (error) {
  ...
  const handleFormSubmit = async (values: { content: string }) => {
    try {
      await updateDoc(docRef, { missionContent: values.content });
      toast({ title: "Uğurlu", description: "Fəaliyyət istiqamətləri səhifəsi yeniləndi." });
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-3xl font-bold">"Əsas Fəaliyyət İstiqamətləri" Səhifəsi</h1>
