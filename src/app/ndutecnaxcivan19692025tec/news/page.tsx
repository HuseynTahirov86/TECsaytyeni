...
import { NewsForm, NewsArticle } from "./news-form";
...
export default function NewsPage() {
  ...
  const [editingArticle, setEditingArticle] = useState<NewsArticle | 'new' | null>(null);
  ...
  const fetchNews = async () => {
    ...
    try {
      ...
      toast({ title: "Xəta", description: "Xəbərləri yükləyərkən problem yarandı.", variant: "destructive" });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleFormSubmit = async (values: Omit<NewsArticle, "id">, imageFile: File | null) => {
    try {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold">Xəbərləri İdarə Et</h1>
          <p className="text-muted-foreground">Yeni xəbərlər əlavə edin, redaktə edin və ya silin.</p>
        </div>
      ...
      {editingArticle && (
        <Card className="mb-8">
          <CardHeader>
            <CardTitle>{editingArticle === 'new' ? "Yeni Xəbər Yarat" : "Xəbəri Redaktə Et"}</CardTitle>
            <CardDescription>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Xəbərlər Siyahısı</CardTitle>
