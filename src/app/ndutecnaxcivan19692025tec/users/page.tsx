...
import { Trash2, Inbox, User, FileOutput } from "lucide-react";
...
export default function UsersAdminPage() {
  ...
  const handleExport = () => {
    const dataToExport = users.map(user => ({
      'Ad, Soyad': user.name,
      'Email': user.email,
      'Təyinat': user.designation,
      'İxtisas': user.specialization,
      'Kurs': user.course,
      'Qeydiyyat Tarixi': user.createdAt ? new Date(user.createdAt).toLocaleDateString('az-AZ') : 'Bilinmir',
    }));
    exportToExcel(dataToExport, "Təlim_İstifadəçiləri");
  };
