...
export default function SecProjectsPage() {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      setIsLoading(true);
      try {
        const q = query(collection(db, "secProjects"));
        ...
        {isLoading ? (
          ...
          projects.map((project) => (
            <motion.div key={project.id} variants={FADE_IN_ANIMATION_SETTINGS}>
              <Link href={`/affiliated-bodies/gymnasium-sec/projects/${project.slug}`} className="group">
