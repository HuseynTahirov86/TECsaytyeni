...
export default function DepartmentDocumentClientPage() {
  ...
  const [documentType, setDocumentType] = useState<"statistics" | "protocol">("statistics");
  ...
  return (
    ...
    >
      <header className="text-center mb-8">
       ...
       <Card className="mb-8">
        <CardContent className="pt-6">
            ...
            onValueChange={(value: "statistics" | "protocol") => setDocumentType(value)}
            className="flex flex-col sm:flex-row gap-4"
            >
                <Label htmlFor="statistics" className="flex-1">
                    <Card className={`cursor-pointer transition-all ${documentType === 'statistics' ? 'border-primary ring-2 ring-primary' : 'hover:border-muted-foreground/50'}`}>
                 ...
                 <Label htmlFor="protocol" className="flex-1">
                    <Card className={`cursor-pointer transition-all ${documentType === 'protocol' ? 'border-primary ring-2 ring-primary' : 'hover:border-muted-foreground/50'}`}>
                        <CardHeader className="flex flex-row items-center gap-4">
      ...
      <div className="p-4 sm:p-8 border rounded-lg bg-card text-card-foreground shadow-lg">
        {documentType === 'statistics' ? <DepartmentStatisticsForm /> : <DepartmentProtocolForm />}
      </div>
    </motion.div>
  );
}
